...
3. `run.cmd`: Gateway(8000), STT(8001), TTS(8003), Archive(8004), Frontend(8081), Admin(8082)과 필요 시 portrait worker를 시작합니다.
...
외부 Nginx에는 Gateway와 브라우저가 읽어야 하는 생성 음성 파일 경로만 공개합니다. Archive API, STT, TTS 합성 API, LLM 및 테스트 UI는 외부에서 404로 차단합니다. 자세한 설정은 [배포 안내](deploy/README.md)를 참고하세요.
...
Gateway API 문서는 실행 후 `http://127.0.0.1:8000/docs`에서 확인할 수 있습니다.
