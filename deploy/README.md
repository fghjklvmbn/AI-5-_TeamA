...
Web/mobile client
  ...
  -> gateway on 192.168.2.75:8010
     -> STT     192.168.2.75:8001
     -> TTS     192.168.2.75:8003
     -> archive 192.168.2.75:8004
     -> LLM     https://developark.duckdns.org/api_memoripal/llm/v1
```

Clients should call only the gateway. The external Nginx routes STT and TTS to
the application host for operational access, but their compute endpoints require
the server-only `MEMORYPAL_MODEL_SERVICE_TOKEN`. CORS is retained as an
additional browser-origin check and is not treated as authentication. Gateway
uses the LAN STT/TTS URLs and the external LLM proxy at
`/api_memoripal/llm/v1`; generated TTS audio is read through the public proxy.
...
Archive APIs and the test TTS UI remain blocked. STT and TTS are routed by
`nginx/memorypal.conf.example`, while transcription and synthesis stay protected
by the shared server-only Bearer token. Generated audio remains publicly readable.
When Archive and TTS return audio URLs, use these environment variables so
browsers receive HTTPS URLs instead of private LAN paths:
