...
from services.tts_service import TTSService, reference_upload_root
...
def test_uploaded_reference_audio_is_in_an_allowed_root(tmp_path, monkeypatch):
    upload_root = tmp_path / "uploads"
    monkeypatch.setenv("MEMORYPAL_TTS_REFERENCE_UPLOAD_DIR", str(upload_root))
    upload_root.mkdir()
    reference = upload_root / "voice.wav"
    reference.write_bytes(b"wave")
    assert reference_upload_root() == upload_root.resolve()
    assert TTSService()._localize_ref_audio(str(reference)) == str(reference.resolve())
