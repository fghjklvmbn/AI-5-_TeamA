...
class VoiceService:
    ...
    @staticmethod
    def delete(db, voice_id, upload_dir, archive_dir=None) -> bool:
        """Delete a managed legacy voice with a filesystem rollback boundary."""
        voice = VoiceRepository.get_internal_by_id(db, voice_id)
        if voice is None:
            return False
        ...
        try:
            ...
            raise ValueError("관리되는 개인화 음성 파일 경로가 아닙니다.") from exc

        tombstone = None
        if audio_path.exists():
            if not audio_path.is_file():
                raise ValueError("개인화 음성 경로가 파일이 아닙니다.")
            tombstone = audio_path.with_name(
                f".{audio_path.name}.{uuid.uuid4().hex}.deleting"
            )
            audio_path.replace(tombstone)
