...
def test_reasoning_capabilities_use_lmstudio_native_model_metadata(monkeypatch):
    ...
    class Response:
        ...
        def json(self):
            return {"models": [{
                "key": "qwen/qwen3.5-4b",
                "loaded_instances": [{"id": "qwen3.5-4b"}],
    ...
    result = asyncio.run(pipeline.reasoning_capabilities("qwen3.5-4b"))
    assert requested == ["https://developark.duckdns.org/api_memoripal/llm/api/v1/models"]
    assert result["thinking_supported"] is True
    assert result["reasoning_efforts"] == ["low", "medium", "high"]
    assert result["default_reasoning"] == "on"


def test_reasoning_capabilities_enable_switch_without_depth_options(monkeypatch):
    pipeline = ModelPipeline(load_settings())
    ...
    class Response:
        ...
        def json(self):
            return {"models": [{
                "key": "qwen/qwen3.5-4b",
                "capabilities": {"reasoning": {
                    "allowed_options": ["off", "on"], "default": "off",
                }},
            }]}
    ...
    class Client:
        async def __aenter__(self): return self
        async def __aexit__(self, *_args): return None
        async def get(self, _url, headers): return Response()
...
def test_completion_thinking_mode_reserves_reasoning_budget(monkeypatch, reasoning_effort):
    ...
    monkeypatch.setattr("memorypal_api.services.pipeline.httpx.AsyncClient", lambda **_kwargs: Client())
    answer = asyncio.run(pipeline._completion(
        original, 0.4, "qwen3.5-4b", thinking_mode=True,
...
def test_request_schemas_default_thinking_mode_off():
    ...
    assert ChatRequest(text="질문").reasoning_effort is None
    assert RegenerateRequest().reasoning_effort is None
...
def test_primary_failure_retries_compact_prompt_with_same_model():
    ...
    assert [model for _, model in calls] == ["qwen3.5-4b", "qwen3.5-4b"]
...
def test_default_voice_uses_configured_url_when_archive_row_is_missing(monkeypatch):
    pipeline = ModelPipeline(replace(
        load_settings(),
        archive_service_token=ARCHIVE_SERVICE_TOKEN,
        model_service_token=MODEL_SERVICE_TOKEN,
        default_voice_audio_url="https://voice.example/default.wav",
        default_voice_reference_text="기본 음성 원문입니다.",
    ))
    calls = []
    ...
    class Client:
        ...
        async def get(self, url, headers):
            ...
            if url.endswith(pipeline.settings.default_voice_id):
                return Response(status_code=404)
            return Response(
                content=b"RIFF-url-reference", headers={"content-type": "audio/wav"},
            )
        async def post(self, url, headers, data, files):
            calls.append(("POST", url, headers))
            assert data["ref_text"] == "기본 음성 원문입니다."
            assert files["ref_audio"] == (
                "default.wav", b"RIFF-url-reference", "audio/wav",
            )
            return Response({"audio_path": "http://127.0.0.1:8003/outputs/fallback.wav"})

    monkeypatch.setattr("memorypal_api.services.pipeline.httpx.AsyncClient", lambda **_kwargs: Client())
    result = asyncio.run(pipeline.synthesize("URL 기본 음성 테스트", None))
    assert result.endswith("/outputs/fallback.wav")
    assert calls[1] == ("GET", "https://voice.example/default.wav", {})


def test_list_voices_includes_configured_default_when_archive_is_empty(monkeypatch):
    pipeline = ModelPipeline(replace(
        load_settings(), default_voice_audio_url="https://voice.example/default.wav",
        default_voice_reference_text="기본 음성 원문입니다.",
    ))
    ...
    monkeypatch.setattr("memorypal_api.services.pipeline.httpx.AsyncClient", lambda **_kwargs: Client())
    voices = asyncio.run(pipeline.list_voices())
    assert voices[0]["id"] == pipeline.settings.default_voice_id
    assert voices[0]["audio_path"] == "https://voice.example/default.wav"
