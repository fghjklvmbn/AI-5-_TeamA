...
EXCLUDED_PATHS = {"/health", "/v1/health", "/docs", "/openapi.json", "/redoc"}
...
class OperationStateManager:
    ...
    async def begin_request(self, request: Request) -> RequestOperation | None:
        ...
        # Client request IDs are useful trace labels but are not trusted as a
        # globally unique operation key. A server UUID prevents two users or
        # routes that reuse a header from sharing one state-machine row.
        ...
        try:
            ...
            logger.exception("Failed to create request operation state")
            return None
    ...
    async def finish_request(
        ...
        # its final action/status event anonymous while failed attempts remain traceable.
