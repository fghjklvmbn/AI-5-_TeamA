...
class Database:
    ...
    def list_rag_embeddings(
        self, *, user_id: str, namespace: str, embedding_model: str,
    ) -> list[sqlite3.Row]:
        return self.fetch_all(
            "SELECT * FROM rag_embeddings WHERE user_id = ? AND namespace = ? "
            "AND embedding_model = ?",
            (user_id, namespace, embedding_model),
        )
    ...
    def search_rag_embeddings(
        ...
        *,
        ...
        """Cosine-search persisted vectors when pgvector is not available."""
        if not vector:
            return []
        query_norm = sum(value * value for value in vector) ** 0.5
        if query_norm == 0:
            return []
        matches: list[dict[str, Any]] = []
        for row in self.list_rag_embeddings(
            user_id=user_id, namespace=namespace, embedding_model=embedding_model,
        ):
            try:
                candidate = [float(value) for value in json.loads(row["vector_json"])]
            except (TypeError, ValueError, json.JSONDecodeError):
                continue
            if len(candidate) != len(vector):
                continue
            candidate_norm = sum(value * value for value in candidate) ** 0.5
            if candidate_norm == 0:
                continue
            similarity = sum(
                left * right for left, right in zip(vector, candidate)
            ) / (query_norm * candidate_norm)
            item = dict(row)
            item["similarity"] = similarity
            matches.append(item)
        matches.sort(key=lambda item: float(item["similarity"]), reverse=True)
        return matches[:max(1, min(100, limit))]
